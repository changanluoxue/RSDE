...
for n in orders:
    '导入路径签名'
    sig_hatW = np.load(f'data/{name}/simulated_data/sig_hatW/sig_hatW_all_order{n}.npy') # (M, J+1, features)
    ...
    if method == 'nonlinear':
        ...
        nn_model.eval()
        paths_v_hat_all[f'order{n}'] = nn_model(torch.from_numpy(sig_hatW_t).float()).detach().squeeze().numpy()  # (M, J+1)
        paths_I_hat_all[f'order{n}'] = simulate_I(paths_v_hat_all[f'order{n}'], paths_dW, J, M)
    if method == 'linear':
        nn_model = signn_l(features=order_feature[n])
        nn_model.load_state_dict(torch.load(f'data/{name}/results/nn_models/nn_sig_model_{method}_order{n}.pth', map_location=device))
        nn_model.eval()
        paths_v_hat = nn_model(torch.from_numpy(sig_hatW_t).float()).detach().numpy()  # (M, J+1, features)
        paths_v_hat_all[f'order{n}'] = np.sum(paths_v_hat*sig_hatW, -1) # (M, J+1)
        paths_I_hat_all[f'order{n}'] = simulate_I(paths_v_hat_all[f'order{n}'], paths_dW, J, M)
